...
export const SeatSelectionView: React.FC<SeatSelectionProps> = ({
  ...
  return (
    <div className="w-full lg:w-7/12 flex flex-col justify-center my-auto">
      ...
      >
        {/* Title & Legend Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-[#232938]">
          <div>
          ...
          <div className="flex items-center space-x-6 text-xs font-semibold tracking-wider">
            <div className="flex items-center space-x-2">
              <div className="w-3.5 h-3.5 rounded-full bg-[#252e47] border border-[#313c59] shadow-sm" />
              <span className="text-gray-300">Available</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3.5 h-3.5 rounded-full bg-[#d4af37] shadow-sm shadow-[#d4af37]/40" />
              <span className="text-[#d4af37] font-bold">Selected</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3.5 h-3.5 rounded-full bg-[#10141f] border border-[#1e2538]" />
        ...
        {/* Stage Component */}
        <div className="w-full max-w-lg mx-auto py-2 bg-gradient-to-r from-[#1c180e] via-[#2c2415] to-[#1c180e] rounded-b-3xl border-t-2 border-[#d4af37]/60 text-center shadow-lg mb-2">
          <span className="font-cinzel text-xs font-black tracking-[0.5em] text-[#d4af37] uppercase">STAGE</span>
        </div>
          ...
          <div className="min-w-[650px] flex flex-col space-y-3">
            {rows.map((row) => (
              <div key={row} className="flex items-center justify-between space-x-3">
                <span className="font-mono text-xs font-bold text-[#d4af37] w-6 text-center">{row}</span>
                  ...
                  {Array.from({ length: cols }, (_, i) => i + 1).map((col) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-7 sm:h-8 rounded-md font-mono text-[10px] sm:text-[11px] font-bold flex items-center justify-center transition-all duration-200 cursor-pointer ${
                          isSelected
                            ? 'bg-gradient-to-r from-[#e6ca65] via-[#d4af37] to-[#b38f44] text-black font-extrabold scale-110 shadow-md shadow-[#d4af37]/50 ring-2 ring-white/30 z-10'
                            : isBooked
                              ? 'bg-[#10141f] text-gray-600 border border-[#1e2538] cursor-not-allowed opacity-60'
                              : 'bg-[#1c2235]/80 hover:bg-[#252e47] text-[#cbd5e1] border border-[#313c59] shadow-sm'
                        }`}
                      >
                        {col}
                      </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-6 border-t border-[#232938] items-center">
          {/* Selected Chips */}
              ...
              {selectedSeats.length === 0 ? (
                ...
                selectedSeats.map((sid) => (
                  <span
                    ...
                    className="px-2.5 py-1 bg-[#d4af37]/15 border border-[#d4af37]/40 rounded-full text-[#e6ca65] font-mono text-xs font-bold flex items-center space-x-1 cursor-pointer hover:bg-[#d4af37]/25 transition group shrink-0"
                  >
                    <span>{sid}</span>
                    <X className="w-3.5 h-3.5 text-[#d4af37] group-hover:text-red-400" />
                  </span>
                ))
              )}
            </div>
          ...
          {/* Ticket Counter */}
          <div className="flex flex-col items-center justify-center space-y-2 border-y lg:border-y-0 lg:border-x border-[#232938] py-4 lg:py-0">
            <span className="text-xs uppercase text-gray-400 font-semibold tracking-wider">Tickets Selected</span>
            <div className="flex items-center space-x-4">
              <button 
                onClick={handleDecrement}
                disabled={selectedSeats.length === 0}
                className="w-10 h-10 rounded-xl bg-black/60 border border-[#d4af37]/30 flex items-center justify-center text-[#d4af37] hover:bg-[#d4af37]/20 disabled:opacity-30 transition cursor-pointer"
              >
                <Minus className="w-5 h-5" />
              </button>
              <span className="font-mono text-2xl font-black text-[#e6ca65] w-8 text-center">
          ...
          <div className="flex flex-col lg:items-end space-y-1 text-center lg:text-right">
            ...
            <span className="font-mono text-3xl font-black text-[#d4af37]">
        ...
        <div className="bg-[#d4af37]/10 p-4 rounded-xl border border-[#d4af37]/30 flex items-center space-x-3 text-xs text-[#e6ca65] text-left">
          <ShieldCheck className="w-5 h-5 text-[#d4af37] shrink-0" />
          <span>Aadhaar verification required in next step. Ensure you have your 12-digit Aadhaar number ready.</span>
        </div>

        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-[#232938]">
          <button 
            onClick={onBack}
          ...
          <button 
            ...
            className={`w-full sm:w-auto px-10 py-3.5 rounded-xl font-cinzel font-bold text-base tracking-widest uppercase shadow-xl transition duration-300 flex items-center justify-center space-x-2 ${
              selectedSeats.length > 0 
                ? 'bg-gradient-to-r from-[#e6ca65] via-[#d4af37] to-[#b38f44] text-black hover:opacity-95 shadow-[#d4af37]/20 cursor-pointer font-bold' 
                : 'bg-[#1e2538] text-gray-500 border border-[#2e374f] opacity-50 cursor-not-allowed font-medium'
            }`}
          >
